...
public class AMDF implements PitchDetector {
	...
	private static final double DEFAULT_MIN_FREQUENCY = 1000.0;
	private static final double DEFAULT_MAX_FREQUENCY = 20000.0;
	...
	public AMDF(float sampleRate, int bufferSize) {
		this(sampleRate, bufferSize, DEFAULT_MIN_FREQUENCY,
				DEFAULT_MAX_FREQUENCY);
	}

	public AMDF(float sampleRate, int bufferSize, double minFrequency,
			double maxFrequency) {
	...
	public PitchDetectionResult getPitch(float[] audioBuffer) {
		...
		int cutoff = (int) Math.round((sensitivity * (maxval - minval))
				+ minval);
		int j = (int) minPeriod;
